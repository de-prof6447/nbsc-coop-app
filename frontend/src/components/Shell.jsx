  ...
  >
    Official Website
          ...
          <div className="ml-auto flex items-center gap-2">
            <Link
              to="/change-password"
              className={`btn-ghost text-sm ${loc.pathname.includes("change-password") ? "ring-2 ring-blue-600" : ""}`}
            >
              Change Password
            </Link>
      ...
      <main className="max-w-5xl mx-auto px-3 py-4">
        <div className="text-xs text-slate-500 mb-3">
          Logged in as <span className="font-semibold text-slate-700">{user.full_name}</span> ({user.role}) — SAP {user.sap_no}
        </div>
        <Outlet />
      </main>
